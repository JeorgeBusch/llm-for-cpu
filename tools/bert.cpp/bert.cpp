...
struct bert_ctx * bert_load_from_file(const char *fname)
{
...

    {
        ...
        model_mem_req += n_layer * (n_embd * ggml_type_sizef(GGML_TYPE_F32)); // ff2_o_b
    ...
    // prepare memory for the weights
    {
        ...
        for (int i = 0; i < n_layer; ++i)
        {
			...
			*/
            ...
            model.tensors["transformer.layer." + std::to_string(i) + ".attention.k_lin.bias"] = layer.k_b;
            model.tensors["transformer.layer." + std::to_string(i) + ".attention.v_lin.weight"] = layer.v_w;
            model.tensors["transformer.layer." + std::to_string(i) + ".attention.v_lin.bias"] = layer.v_b;
            model.tensors["transformer.layer." + std::to_string(i) + ".attention.out_lin.weight"] = layer.o_w;
            model.tensors["transformer.layer." + std::to_string(i) + ".attention.out_lin.bias"] = layer.o_b;
            model.tensors["transformer.layer." + std::to_string(i) + ".sa_layer_norm.weight"] = layer.ln_att_w;
            model.tensors["transformer.layer." + std::to_string(i) + ".sa_layer_norm.bias"] = layer.ln_att_b;

            model.tensors["transformer.layer." + std::to_string(i) + ".ffn.lin1.weight"] = layer.ff_i_w;
            model.tensors["transformer.layer." + std::to_string(i) + ".ffn.lin1.bias"] = layer.ff_i_b;

            model.tensors["transformer.layer." + std::to_string(i) + ".ffn.lin2.weight"] = layer.ff2_i_w;
    ...
    // load weights
    {
        ...
        while (true)
        {
            ...
            {	
				printf("Tensor has %d model has %d",ggml_nelements(tensor), nelements );
    ...
    // Calculate space requirements for setting up context buffers later
    {
        ...
        bert_eval(new_bert, 1, tokens, 4, nullptr); // error here?
...
{	
    ...
    for (int ba = 0; ba < n_batch_size; ba++)
    {
        ...
        inpL = ggml_add(ctx0,
                        ...
                        inpL);
		//printf("Word embedding done, iter %d / %d\n", ba, n_batch_size);
        // embd norm
        {
        ...
        for (int il = 0; il < n_layer; il++)
        {
            ...
            // self-attention
            {
				//printf("Starting layer %d, iter %d / %d\n", il, ba, n_batch_size);
			...
			//printf("Attention Normalization, iter %d / %d\n", ba, n_batch_size);
            // attention norm
            {
			...
			//printf("FC layer 2, iter %d / %d\n", ba, n_batch_size);
			cur = ggml_mul_mat(ctx0, model.layers[il].ff2_i_w, cur);
            cur = ggml_add(ctx0,
                           ggml_repeat(ctx0, model.layers[il].ff2_i_b, cur),
                           cur);
            cur = ggml_gelu(ctx0, cur);
			...
			printf("ff shape: %d %d %d %d\n",model.layers[il].ff_o_b->ne[0], model.layers[il].ff_o_b->ne[1], model.layers[il].ff_o_b->ne[2], model.layers[il].ff_o_b->ne[3] );
			printf("cur shape: %d %d %d %d\n", cur->ne[0], cur->ne[1], cur->ne[2], cur->ne[3]);
            cur = ggml_add(ctx0,
			...
			*/
            // output norm
            {
			//printf("Layer Normalization, iter %d / %d\n", ba, n_batch_size);
                cur = ggml_norm(ctx0, cur);
        ...
        // pretty_print_tensor(dat, output->ne, output->nb, output->n_dims - 1, "");
            ...
            ggml_graph_print(&gf);
        #endif
		//printf("Finalize Results, iter %d / %d\n", ba, n_batch_size);
        if (!mem_req_mode) {
